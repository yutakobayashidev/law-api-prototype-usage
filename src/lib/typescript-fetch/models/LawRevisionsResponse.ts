 ...
 * 法令履歴一覧取得APIレスポンス
 * @export
 * @interface LawRevisionsResponse
 */
export interface LawRevisionsResponse {
    /**
     * 
     * @type {LawInfo}
     * @memberof LawRevisionsResponse
     */
    lawInfo: LawInfo;
    /**
     * 版一覧
...
/**
 * Check if a given object implements the LawRevisionsResponse interface.
 */
export function instanceOfLawRevisionsResponse(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "lawInfo" in value;
    isInstance = isInstance && "revisions" in value;

    return isInstance;
}

export function LawRevisionsResponseFromJSON(json: any): LawRevisionsResponse {
    return LawRevisionsResponseFromJSONTyped(json, false);
}

export function LawRevisionsResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): LawRevisionsResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'lawInfo': LawInfoFromJSON(json['law_info']),
        'revisions': ((json['revisions'] as Array<any>).map(RevisionInfoFromJSON)),
    };
}

export function LawRevisionsResponseToJSON(value?: LawRevisionsResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'revisions': ((value.revisions as Array<any>).map(RevisionInfoToJSON)),
    };
}
